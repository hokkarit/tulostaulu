...
1. Täytä kotijoukkueen ja vierasjoukkueen nimet (enintään 8 merkkiä kumpikin) sekä valitse molempien logot (PNG, JPG tai JPEG) — joko käsin tai yllä kuvatusta logotietokannasta.
   - **Pienpeli-tila:** kytke "Pienpeli-tila" päälle, kun samalla kentällä pelataan yhtäaikaa kaksi peliä eli näytöllä on neljä joukkuetta. Tällöin logoille ei ole näytöllä tilaa, joten logokentät piilotetaan eikä logoja kirjoiteta konfiguraatioon. Sen sijaan täytä myös toisen pelin koti- ja vierasjoukkueen nimet (enintään 8 merkkiä kumpikin); ne tallennetaan tiedostoihin `home/nimi2.txt` ja `guest/nimi2.txt` ensimmäisen pelin `home/nimi.txt`- ja `guest/nimi.txt`-tiedostojen rinnalle.
